...
const waitForMetadataLoad = (/** @type {HTMLVideoElement} */ videoEl) => new Promise((resolve, reject) => {
  videoEl.addEventListener('loadedmetadata', () => resolve(true), { once: true, passive: true });
  videoEl.addEventListener('error', () => reject(false), { once: true, passive: true });
});

const waitForSeek = (/** @type {HTMLVideoElement} */ videoEl) => new Promise((resolve) => {
  videoEl.addEventListener('seeked', resolve, { once: true, passive: true });
});

async function computePHash() {

  /** @type {HTMLVideoElement | null} */
  const video = document.querySelector('video[src]:not([src=""])') || document.querySelector('video');
  if (!video) {
    alert(
      "no video found\n" +
      "do you need to click the play button?\n" +
      "if the video is inside an iframe, go the iframe's source"
    );
    const iframeSrc =
      /** @type {HTMLIFrameElement | null} */
      (document.querySelector('*[class*="player"] iframe, *[class*="video"] iframe'))
        ?.src;
    if (iframeSrc && confirm('Navigate to iframe source?\n' + iframeSrc)) {
  ...
  if (video.readyState < HTMLMediaElement.HAVE_METADATA) {
    try {
      await Promise.race([
        waitForMetadataLoad(video),
        wait(10000).then(() => Promise.reject()),
      ]);
    } catch (error) {
  ...
  if (!document.querySelector('style#compute-phash')) {
    const style = document.createElement('style');
    style.id = 'compute-phash';
...
`;
  ...
  resultBox.id = 'compute-phash';
  document.body.prepend(resultBox);

  const computeStatus = document.createElement('h1');
  computeStatus.innerText = `Compute PHash: Starting...`;
  resultBox.append(computeStatus);
  ...
  /** @returns {[sprite: HTMLCanvasElement, spriteCtx: CanvasRenderingContext2D]} */
  const makeSprite = () => {
    ...
    return [sprite, spriteCtx];
  };

  /**
   * Test if the canvas will be tainted by drawing the video element on it.
   * @returns {Error | null}
   */
  const securityTest = () => {
    const [canvas, canvasCtx] = makeSprite();
    try {
  ...
  let testError = securityTest();
  if (testError !== null && video.crossOrigin !== 'anonymous') {
    // enable CORS on the video element and reload the source
    const srcURL = new URL(video.src);
    if (['http:', 'https:'].includes(srcURL.protocol)) {
      console.log('setting `crossOrigin=anonymous` and reloading video');
      video.crossOrigin = 'anonymous';
      srcURL.searchParams.set('_cors', '1');
      video.src = srcURL.toString();
      try {
        await waitForMetadataLoad(video);
        await wait(500);
        testError = securityTest();
      } catch (error) {
        testError = 'cross-origin video refused to load';
      }
    }
  }

  if (testError !== null) {
    computeStatus.innerText = `Unable to compute PHash: ${testError}`;
    console.error('Unable to compute PHash:', testError);
    return;
  }

  const [sprite, spriteCtx] = makeSprite();
  // insert the sprite as a visual progress bar
  resultBox.append(sprite);
  ...
  for (let index = 0; index < chunkCount; index++) {
    ...
    // Make sure the video remains paused.
    if (!video.paused)
      video.pause();
  ...
  try {
    ...
    spriteImage.width = sprite.width;
    spriteImage.height = sprite.height;
  ...
  const phash = async (sprite) => {
    ...
    let pixels;
    try {
      pixels = hashCtx.getImageData(0, 0, hashCanvas.width, hashCanvas.height);
    } catch (error) {
      computeStatus.innerText = `Unable to compute PHash: ${error}`;
      console.error('Unable to compute PHash:', error);
      return;
    }
